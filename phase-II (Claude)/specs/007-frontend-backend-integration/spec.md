...
**Independent Test**: Can be fully tested by configuring the BACKEND_URL in the frontend's .env.local file and verifying that API calls to the backend succeed, delivering the ability to create, read, update, and delete todo items.
...
2. **Given** the backend API is running, **When** the frontend makes API calls using the configured BACKEND_URL, **Then** the API responses are correctly processed and displayed in the UI

---

### User Story 2 - Configure Backend URL Environment Variable (Priority: P2)

As a developer, I want to configure the BACKEND_URL in the frontend's .env.local file so that the application can connect to the appropriate backend service based on the environment.
...
**Independent Test**: Can be tested by setting up the BACKEND_URL environment variable in the .env.local file and verifying that the frontend correctly uses this URL for all API calls.

**Acceptance Scenarios**:

1. **Given** the .env.local file exists with BACKEND_URL configured, **When** the frontend application starts, **Then** all API calls use the specified backend URL
2. **Given** the .env.local file has a valid BACKEND_URL, **When** API calls are made from the frontend, **Then** the requests are properly directed to the backend service
...
- What happens when the BACKEND_URL is misconfigured or points to an unavailable service?
...
- **FR-001**: System MUST configure the BACKEND_URL in the frontend's .env.local file to establish communication with the backend API
- **FR-002**: System MUST use the configured BACKEND_URL to make API calls from the Next.js frontend to the FastAPI backend
...
- **FR-007**: System MUST validate that the configured BACKEND_URL is accessible before attempting API calls
...
- **Frontend Configuration**: Environment variables and settings that control frontend behavior, specifically the BACKEND_URL
...
- **SC-003**: Frontend application successfully connects to backend API with the configured BACKEND_URL on first attempt in 95% of deployments
